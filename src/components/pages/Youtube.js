...
function YouVideo() {
  ...
  const search = (query) => {
    ...
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=${query}&type=video&key=AIzaSyDuo94dObpRon84kNFdxW1MCGWv6bUTzCg
      `,
      requestOptions
    )
  ...
  useEffect(() => {
    ...
    fetch(
      "https://youtube.googleapis.com/youtube/v3/search?key=AIzaSyDuo94dObpRon84kNFdxW1MCGWv6bUTzCg&part=snippet&maxResults=20&q=Cat&typevideo=video&key=AIzaSyDuo94dObpRon84kNFdxW1MCGWv6bUTzCg",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => setVideos(result.items))
